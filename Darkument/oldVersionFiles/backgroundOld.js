...
async function checkFileUrlPermission() {
  try {
    ...
    try {
      ...
      console.warn("Cannot check file URL permissions:", fallbackError);
      return false;
    }
  }
}
...
chrome.runtime.onInstalled.addListener(async (details) => {
  ...
  if (details.reason === 'install') {
    try {
      ...
      // Then create settings page after a short delay
      setTimeout(async () => {
        try {
          const settingsTab = await chrome.tabs.create({ url: settingsUrl });
          const extensionsTabId = settingsTab.id;
          ...
          const pollPermissions = async () => {
            try {
              ...
              if (isAllowed && extensionsTabId !== undefined) {
                // Close settings tab
                try {
                  await chrome.tabs.remove(extensionsTabId);
                } catch (tabError) {
                  // Tab might already be closed
                  console.log("Settings tab already closed");
                }
        ...
        } catch (settingsTabError) {
          console.error("❌ Failed to create settings tab:", settingsTabError);
        }
      }, 2000);

    } catch (welcomeTabError) {
      console.error("❌ Failed to create welcome tab:", welcomeTabError);
    }
  }
});
